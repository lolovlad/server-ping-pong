...
from Class.NeurolNet import NetOptimaze
from Core.DataContext import DataContext


class NewGoods(Observer):
    def update(self, subject):
        command = subject[0]
        if command["Type_command"] == "Server" and command["Information"] == "Goods":
            command = subject.pop()
            list_free = DataContext().MapStorage.list_model()



'''class NewGoods(AbstractHandler):
    def gen_sort(self, flag):
        if flag == 1:
            list_qr = create_command("SortTask", [[0, 2]])
            return list_qr, 2
        else:
            list_qr = create_command("Loading", "Loading")
            return list_qr, flag
...
class NewTaskSort(AbstractHandler):
    def handle(self, request):
...
class InfoTime(AbstractHandler):
...
class RefreshFlag(AbstractHandler):
    def handle(self, request):
